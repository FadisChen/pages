...
獨立的原生 HTML／CSS／ES Modules 酒館交談遊戲。你扮演酒保，可選擇透過 Gemini Live API 語音交談，或以一般 Gemini API 文字串流與會記得你的奇幻旅人交談。正式入口為 `index.html`；`prototype.html` 僅保留為 throwaway 設計比較稿，正式頁面不引用它，也不引用既有 `web` 專案的 JavaScript、CSS、儲存鍵或元件。

## 特色

- **雙模式故事**：「療癒夜話」（陪伴、近況）與「灰燼群像」（線索、祕密），各角色人設與記憶會依模式切換。
- **雙互動模式**：語音模式以 WebSocket 串接 Gemini Live API；文字模式以整理記憶模型的 `streamGenerateContent` 串流回覆。
...
- **角色人設**：十三位角色的立繪、聲線、姓名不變，但 `personas.js` 為每人準備兩套 `persona` 文字與種子記憶——療癒夜話偏純陪伴、近況閒聊；灰燼群像則讓每位角色捲入「灰燼商隊」失蹤懸案，各自握有需靠信任度逐步透露的線索。
...
案件簿會把已確認線索整理成三條可釘選的調查路線：失蹤路線、記憶灰貨物、內應與金流。玩家可用案件簿提交線索卡組合，正確證據鏈由本地規則驗證；完成任兩條路線並取得倖存者證詞後，才能提交最終指控。角色信任由有效交談與明確分享穩定累積，錯誤指控只會影響玩家選定的結案角色，不會改寫故事正典。
...
進入遊戲後於「設定」輸入 Gemini API key。預設只放在 `sessionStorage`（關閉分頁即清除）；只有勾選「記住」才會寫入 `localStorage`。互動模式、Live 與記憶整理模型名稱都可在全域設定中調整；文字互動使用記憶整理模型。
