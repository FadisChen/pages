...
const GESTURE_SYSTEM_PROMPT = "你可以在回覆開始前，依照即將說出的內容最多呼叫一次 play_avatar_gesture：肯定用 nod、否定用 shake_head、招呼或道別用 wave、解釋介紹用 present、疑問思考用 tilt_head、道謝或道歉用 bow、不確定用 shrug、感謝或關心用 hand_on_chest、請對方靠近或繼續用 beckon、收到指示或正式確認用 salute。沒有適合情境就不呼叫，不要描述工具或動作。";
...
function buildSystemInstruction(userSystemPrompt) {
  ...
  return `${REQUIRED_SYSTEM_PROMPT_PREFIX}${personality}。${REQUIRED_SYSTEM_PROMPT}${GESTURE_SYSTEM_PROMPT}`;
}
