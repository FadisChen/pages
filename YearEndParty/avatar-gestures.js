...
const AVATAR_GESTURE_TOOL = Object.freeze({
  ...
  description: "Choose one natural gesture for the response: nod for agreement, shake_head for disagreement, wave for greeting or goodbye, present for explaining, and tilt_head for a question or thought. At most once per response; do not call it when no gesture fits.",
  parameters: {
    type: "OBJECT",
...
function normalizeAvatarGesture(args) {
  if (!args || typeof args !== "object" || Array.isArray(args) || !AVATAR_GESTURES.includes(args.gesture)) {
    return { ok: false, error: "Use one supported gesture: nod, shake_head, wave, present, or tilt_head." };
  }
...
// These are local rotations added after the existing natural pose.
