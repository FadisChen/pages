...
## 浮動視窗（切換分頁時 Avatar 仍浮在最上層）

`stage.html` 右上角控制台（設定齒輪打開）的「開始對話」按鈕下面多一顆「浮動視窗」按鈕（`documentPictureInPicture`，Chrome/Edge 116+ 才支援，不支援的瀏覽器按鈕不會出現，其餘功能不受影響）。按下後會把整塊 Avatar 畫面（canvas、狀態文字）搬進一個獨立、永遠置頂、可拖曳可縮放的小視窗，控制台也會跟著自動關閉，用途是尾牙現場要在同一台投影端筆電切到小遊戲、抽獎等其他分頁時，Nami 仍浮在畫面最上層。

使用方式與限制：

- 先在 `stage.html` 分頁按下浮動視窗按鈕（需要使用者點擊才能觸發），小遊戲、抽獎等網頁請**另外開新分頁**顯示，不要直接把 `stage.html` 這個分頁導覽到別的網址——瀏覽器規範是「開啟浮動視窗的那份文件被關閉或導覽走，浮動視窗就會跟著關閉」。
- 連線、播放、狀態機邏輯完全沒有搬動，一樣跑在原本的 `stage.html` 分頁裡；浮動視窗只是把已經在跑的 canvas 重新掛到另一個永遠可見的小視窗，所以配對、Gemini 連線、語音播放都不受影響，切分頁也不會中斷。
- 再按一次按鈕（或直接關掉小視窗）會把畫面搬回原本位置。
