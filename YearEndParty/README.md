...
`stage.html` 右上角設定齒輪旁多一顆浮動視窗圖示按鈕（`documentPictureInPicture`，Chrome/Edge 116+ 才支援，不支援的瀏覽器按鈕不會出現，其餘功能不受影響）。按下後會把整塊 Avatar 畫面（canvas、狀態文字）搬進一個獨立、永遠置頂、可拖曳可縮放的小視窗，用途是尾牙現場要在同一台投影端筆電切到小遊戲、抽獎等其他分頁時，Nami 仍浮在畫面最上層。再按一次會搬回原本位置，動畫不會因此卡住。
