...
function formatSessionContext({ now = new Date(), locale, timeZone, location = null } = {}) {
...
function collectSessionContext({ now = new Date(), locale, timeZone, navigatorObject = globalThis.navigator, timeoutMs = LOCATION_TIMEOUT_MS } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    try { geolocation.getCurrentPosition(finish, () => finish(null), { enableHighAccuracy: false, maximumAge: 300000, timeout: timeoutMs }); }
    catch (_) { finish(null); }
  });
}

export { collectSessionContext, formatSessionContext };
